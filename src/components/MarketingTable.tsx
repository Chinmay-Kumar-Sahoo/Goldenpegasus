              ...
              {loading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <tr key={i} className="border-b border-[#1a1a1a]">
                    {Array.from({ length: 16 + (showPrimaryEmployeeColumn ? 1 : 0) + (showBackupEmployeeColumn ? 1 : 0) + (showEmployeeColumn ? 1 : 0) + (isAdmin ? 1 : 0) + (!readOnly ? 1 : 0) }).map((_, j) => (
                      <td key={j} className="px-4 py-4"><div className="skeleton h-4 w-full" /></td>
                    ))}
                  </tr>
                ))
              ) : error ? (
                <tr><td colSpan={17 + (showPrimaryEmployeeColumn ? 1 : 0) + (showBackupEmployeeColumn ? 1 : 0) + (showEmployeeColumn ? 1 : 0) + (isAdmin ? 1 : 0) + (!readOnly ? 2 : 0)} className="px-4 py-12 text-center">
                ...
                <tr><td colSpan={18 + (showPrimaryEmployeeColumn ? 1 : 0) + (showBackupEmployeeColumn ? 1 : 0) + (showEmployeeColumn ? 1 : 0) + (isAdmin ? 1 : 0) + (!readOnly ? 2 : 0)} className="px-4 py-12 text-center text-[#71717a] text-sm">No records found.</td></tr>
              ) : (
                paginated.map(rec => (
